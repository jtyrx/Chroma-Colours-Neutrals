'use client'

import type Color from 'colorjs.io'
import {useMemo} from 'react'

type OklchColorFieldProps = {
  label: string
  valueText: string
  parsed: Color | null
  onTextChange: (text: string) => void
  onPickSrgbHex: (hex: string) => void
  hint?: string
}

/** OKLCH-first text input plus native sRGB swatch for quick anchors (Evil Martians–style UX without their npm package). */
export function OklchColorField({
  label,
  valueText,
  parsed,
  onTextChange,
  onPickSrgbHex,
  hint,
}: OklchColorFieldProps) {
  const hex = useMemo(() => {
    if (!parsed) return '#888888'
    try {
      return parsed.to('srgb').toString({format: 'hex'})
    } catch {
      return '#888888'
    }
  }, [parsed])

  const valid = parsed !== null

  return (
    <div className="space-y-2">
      <label className="text-sm font-medium text-white/80">{label}</label>
      <div className="flex flex-wrap items-stretch gap-2">
        <input
          type="color"
          value={hex}
          onChange={(e) => onPickSrgbHex(e.target.value)}
          className="h-11 w-14 cursor-pointer rounded-xl border border-white/12 bg-black/30 p-1"
          aria-label={`${label} sRGB picker`}
        />
        <input
          type="text"
          value={valueText}
          onChange={(e) => onTextChange(e.target.value)}
          placeholder="oklch(72% 0.12 260) or #c0ffee"
          spellCheck={false}
          className={`min-w-[12rem] flex-1 rounded-xl border bg-black/25 px-3 py-2 font-mono text-sm text-white placeholder:text-white/35 focus:outline-none focus:ring-2 focus:ring-white/20 ${
            valid ? 'border-white/12' : 'border-amber-500/50'
          }`}
        />
      </div>
      {hint ? <p className="text-xs text-white/45">{hint}</p> : null}
    </div>
  )
}
